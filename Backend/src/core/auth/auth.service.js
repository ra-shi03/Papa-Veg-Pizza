...
import { StoreManager } from "../../modules/food/franchise/models/storeManager.model.js";
...
export const adminLogin = async ({ email, mobile, password } = {}, allowedRoles = null) => {
  ...
  let storeId = activeUserRole?.storeId?.toString() || null;
  let franchiseId = activeUserRole?.franchiseId?.toString() || null;
  let storeName = null;
  let storeCode = null;
  ...
  if (storeId && !storeName) {
    try {
      ...
      if (storeDoc) {
        ...
        if (!storeCode) storeCode = storeDoc.code || null;
      }
    } catch (e) {
      // ignore
    }
  ...
  if (franchiseId) {
    try {
      const franchiseDoc = await FoodFranchise.findById(franchiseId).lean();
      if (franchiseDoc) {
        franchiseName = franchiseDoc.name || franchiseDoc.companyName || null;
      }
    } catch (e) {
      // ignore
    }
  ...
  const userResponse = {
    id:          user._id,
    _id:         user._id,
    name:        profile ? `${profile.firstName || ''} ${profile.lastName || ''}`.trim() : user.name || "",
    email:       user.email,
    mobile:      user.mobile,
    role,
